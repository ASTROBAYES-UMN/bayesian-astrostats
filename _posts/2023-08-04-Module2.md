...
$P(X=x) = \theta^x (1-\theta)^{1-x} \rm{for} x \in \{0,1\},$

which is the pmf corresponding to the *Bernoulli distribution*.

This is a useful distribution for binary random variables. We write $X|\theta \sim \text{Bernoulli} (\theta)$ or just $X \sim \text{Bernoulli}(\theta)$.
